...
class ExtFieldHandler<T> {
  ...
  bool checkDuplicatedPath(String path, T value) => _values.entries
      .where((e) =>
          e.key != path &&
          p.join(p.dirname(path), value.toString()) ==
              p.join(p.dirname(e.key), e.value.toString()))
      .isNotEmpty;
}
...
extension ExFile on FileSystemEntity {
  ...
  bool get newNameDuplicate =>
      _newNameHandler.checkDuplicatedPath(path, newName);
